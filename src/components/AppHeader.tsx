...
function HeaderBrandMark() {
  ...
  if (useFallback) {
    return (
      <span
        className={cn(
          "inline-flex shrink-0 items-center font-black tracking-tight text-primary drop-shadow-sm",
          "text-xs sm:text-sm",
        )}
        aria-hidden
      >
      ...
      >
        {user ? (
          ...
          >
            ...
            <span
              aria-hidden
              className="h-20 w-[2px] shrink-0 rounded-full bg-linear-to-b from-primary/0 via-primary/40 to-primary/0 sm:h-24"
            />
              ...
              <div className="min-w-0">
                <div className="flex flex-wrap items-center justify-end gap-1 sm:gap-1.5">
                  <Link
                    ...
                    className="mr-0.5 inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-primary/30 bg-primary-soft/60 text-base transition-transform hover:scale-105 sm:h-8 sm:w-8 sm:text-lg"
                  >
                    {profile?.avatar ?? "🙂"}
                  </Link>
                  <p
                    className={cn(
                      "max-w-[7.25rem] truncate text-right font-bold leading-tight text-foreground transition-[font-size,max-width] duration-300 sm:max-w-[8.5rem]",
